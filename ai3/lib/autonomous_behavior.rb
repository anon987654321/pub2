...
class AutonomousBehavior
  ...
  def calculate_deadline_urgency(deadline)
    ...
    when 3600..14_400 then 25 # 4 hours
    when 14_400..86_400 then 10 # 24 hours
    else 5
    end
  end
  ...
  def balance_cognitive_load
    ...
    return unless high_cognitive_tasks.size > @tasks.size / 3
  ...
  def execute_task(task)
    ...
    return unless %i[completed failed].include?(task[:status])
  ...
  def update_performance_metrics(task, completion_time, success)
    ...
    cognitive_cost = estimate_cognitive_cost(task)
    return unless success && completion_time > 0
  ...
  def optimize_llm_selection
    ...
    @task_history.each do |task|
      ...
      next unless task[:completed_at] && task[:started_at]
  ...
  def get_model_performance(model)
...

    {
      ...
      avg_completion_time: model_tasks.sum do |t|
        (t[:completed_at] - t[:started_at])
      rescue StandardError
        0
      end / model_tasks.size
    }
  end
